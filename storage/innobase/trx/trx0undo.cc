...
void trx_undo_insert_cleanup(trx_undo_ptr_t *undo_ptr, bool noredo) {
  ...
  ut_ad(rseg);
    ...
    /* Logically undo_ptr->rseg and undo_ptr->undo shouldn't be NULL, but
     *  let's guard against bad input in production build and handle
     *      the error gracefully. */
  if (NULL == rseg || NULL == undo) {
    fprintf(stderr,
            "InnoDB: Error: trx_undo_insert_cleanup(): "
            "undo_ptr->rseg or undo_ptr->insert_undo is NULL. "
            "undo_ptr: %p, rseg: %p, insert_undo: %p\n",
            undo_ptr, rseg, undo);
    return;
  }
