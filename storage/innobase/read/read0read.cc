...
#include "my_securec.h" /* Needs to be the first included header */
...
void ReadView::ids_t::reserve(ulint n) {
  ...
  if (p != NULL) {
    void* ret_p = my_memmove(m_ptr,
                             (capacity()*sizeof(value_type)),
                             p, size() * sizeof(value_type),
                             NULL);

    if (NULL == ret_p) {
      ib::error() << "memmove violation: read0read.cc:" <<  __LINE__;
    }
    ut_ad(ret_p != NULL);
    UT_DELETE_ARRAY(p);
  }
...
void ReadView::ids_t::insert(value_type value) {
  ...
  if (ub == end) {
    ...
    // Space (bytes) left: num_elements_writable * size_per_element
    // Note the copying starts at ub+1 element.
    ulint num_elems_writable = std::distance(ub+1, data()+capacity()-1) + 1;
    ulint bytes_writable = num_elems_writable * sizeof(value_type);

    ut_ad(num_elems_writable <= capacity());
    /* Note: Copying overlapped memory locations. */
    void *ret_p = my_memmove(ub + 1,
                            bytes_writable,
                            ub, n,
                            NULL);
    if (NULL == ret_p) {
...
ReadView::ReadView()
    : m_low_limit_id(),
      ...
      m_ids(),
      m_low_limit_no() {
  ut_d(my_memset(&m_view_list, sizeof(m_view_list), 0x0,
      sizeof(m_view_list), NULL));
...
void ReadView::copy_trx_ids(const trx_ids_t &trx_ids) {
  ...
  if (m_creator_trx_id > 0) {
    ...
    ret_p = my_memmove(p,
                       (m_ids.capacity()*sizeof(ids_t::value_type)),
                        &trx_ids[0], n,
                        NULL);
    ...
    if (n > 0) {
      ret_p = my_memmove(p + i,
                        ((m_ids.capacity()-i)*sizeof(ids_t::value_type)),
                        &trx_ids[i + 1], n,
                        NULL);
