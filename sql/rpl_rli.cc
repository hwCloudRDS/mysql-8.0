...
int Relay_log_info::relay_log_number_to_name(uint number,
  ...
  if (strLen >= (FN_REFLEN - 1)) {
    sql_print_error("Failed to make relay log channel with base name "
                    "%s - relay log channel base name is too long "
                    "(length %zu)", relay_log_basename_channel, strLen);
    rc = 1;
  } else {
    size_t destMax = FN_REFLEN - strLen - 1;

    *str++= '.';
    /* Reach here: destMax couild not be 0 and str could not be NULL */
    if (my_sprintf(str, destMax, "%06u", number) < 0) {
      sql_print_error("Failed to make relay log channel with base name "
                    "%s and number %u - Allowed number of digits "
                    "for the number: %zu",
                    relay_log_basename_channel, number, (destMax - 1));
      rc = 1;
    }
  }
  return rc;
