...
int MYSQL_BIN_LOG::find_next_relay_log(char log_name[FN_REFLEN + 1]) {
  ...
  if (error == 0) {
    ...
    if (error == 0) {
      char *ret= my_strcpy(log_name, (FN_REFLEN + 1), info.log_file_name,
                           NULL);
      if (ret == NULL) {
        sql_print_error("Failed to copy relay log name %s",
                       info.log_file_name);
        error = 1;
      }
    }
...
class Row_data_memory {
  ...
  */
  ...
  // Copy and assignment is never meant to be used for this class
  #define UNUSED(x) (void)x

  Row_data_memory(const Row_data_memory &other)
  {
     DBUG_ASSERT(0);
     UNUSED(other);
  ...
  Row_data_memory &operator=(const Row_data_memory &other)
  {
    DBUG_ASSERT(0);
    UNUSED(other);
    return *this;
