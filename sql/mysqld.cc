...
static int flush_auto_options(const char *fname) {
  ...
  (void) my_b_printf(&io_cache, "%s\n", "[auto]");
  (void) my_b_printf(&io_cache, "server-uuid=%s\n", server_uuid);
...
#endif
{
  ...
  */
  ...
  sql_print_information("[HUAWEISQL-DEBUG] secure C library: '%s'", SECURE_C_LIBRARY);
